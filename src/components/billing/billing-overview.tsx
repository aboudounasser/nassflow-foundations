...
export function BillingOverviewBanner({
  ...
  const quotaPct = Math.min(100, Math.round((totalAiCalls / plan.includedAiCalls) * 100));
  ...
  return (
    <Card className="border-border bg-card p-6">
      ...
      <div className="mt-4 grid min-w-0 grid-cols-2 gap-3 @3xl:grid-cols-4">
        ...
        <Tile
          value={formatEuro(plan.pricePerMonth, 0)}
      ...
      <div className="mt-5">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          ...
          <p className="text-[12px] tabular-nums text-muted-foreground">
            {totalAiCalls.toLocaleString("fr-FR")} / {plan.includedAiCalls.toLocaleString("fr-FR")}{" "}
            ({quotaPct} %)
          </p>
        </div>

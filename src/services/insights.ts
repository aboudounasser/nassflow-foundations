/**
 * Service Layer du module Insights : agrégations cross-module.
...
function missionsByStatus(): { status: string; count: number }[] {
...
function missionsCompletedByWeek(weeks = 6): { week: string; count: number }[] {
...
function pipelineValueByStage(): { stage: string; value: number }[] {
...
function integrationsByStatus(): { status: string; count: number }[] {
...
/** Période des graphiques temporels, pilotée par la vue. */
...
export interface InsightsData {
  overview: ReturnType<typeof insightsOverview>;
  missionsByStatus: ReturnType<typeof missionsByStatus>;
  missionsCompletedByWeek: ReturnType<typeof missionsCompletedByWeek>;
  workflowSuccessRateByDay: ReturnType<typeof workflowSuccessRateByDay>;
  averageConfidenceByAgent: ReturnType<typeof averageConfidenceByAgent>;
  pipelineValueByStage: ReturnType<typeof pipelineValueByStage>;
  integrationsByStatus: ReturnType<typeof integrationsByStatus>;
}

export async function getInsights(_scope: Scope, period: InsightsPeriod): Promise<InsightsData> {
  return delay({
    overview: insightsOverview(),
    missionsByStatus: missionsByStatus(),
    missionsCompletedByWeek: missionsCompletedByWeek(period.weeks),
    workflowSuccessRateByDay: workflowSuccessRateByDay(period.days),
    averageConfidenceByAgent: averageConfidenceByAgent(),
    pipelineValueByStage: pipelineValueByStage(),
    integrationsByStatus: integrationsByStatus(),
  });
}
